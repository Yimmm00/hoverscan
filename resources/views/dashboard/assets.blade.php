<div id="view-panel-assets" class="space-y-8 tab-panel-node hidden animate-fade-in">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-4 gap-8">
        ...
        <div class="xl:col-span-1 p-8 rounded-[2.5rem] border border-slate-200/80 dark:border-white/5 bg-white dark:bg-[#0c0e14] h-fit shadow-sm">
            ...
            <form id="register-bridge-form" class="space-y-5 text-xs font-bold uppercase text-slate-600 dark:text-slate-400">
                ...
                <div>
                    ...
                    <input type="text" name="name" required placeholder="e.g. Batang Rajang Bridge" class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-[#080a0f] text-slate-900 dark:text-white outline-none">
                </div>

                <div>
                    <label class="text-[9px] text-slate-400 dark:text-slate-500 mb-1.5 block font-black">Jurisdiction Division</label>
                    <select name="district" class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-[#080a0f] text-slate-900 dark:text-white outline-none cursor-pointer">
                ...
                <div>
                    <label class="text-[9px] text-slate-400 dark:text-slate-500 mb-1.5 block font-black">GPS Matrix (LAT, LNG)</label>
                    <input type="text" name="location_coords" required placeholder="e.g. 2.5321, 111.8432" class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-[#080a0f] text-slate-900 dark:text-white outline-none font-mono">
                </div>
        ...
        <div class="xl:col-span-2 border border-slate-200/80 dark:border-white/5 rounded-[2.5rem] overflow-hidden bg-white dark:bg-[#0c0e14] h-[550px] overflow-y-auto custom-scrollbar shadow-sm">
...
@push('view-scripts')
<script>
    // 1. GIS Mapping Configuration Logic Block Engine Closure
    let gisMapInstance = null;

    // Track navigation tab switching to load the map context on-demand
    ...
    function initializeInfrastructureMap() {
        ...
        gisMapInstance = new maplibregl.Map({
            ...
            style: initialStyle, 
            center: [111.5000, 2.5000], // Sarawak Central Focus
            zoom: 6.2,
        ...
        // Render current markers from preloaded blade rows elements
    ...
    function bindRowTelemetryMarker(row) {
        ...
        row.addEventListener('click', () => {
            document.querySelectorAll('.asset-row-node').forEach(r => r.classList.remove('bg-slate-100', 'dark:bg-white/5'));
            row.classList.add(document.documentElement.classList.contains('dark') ? 'dark:bg-white/5' : 'bg-slate-100');
            gisMapInstance.flyTo({ center: [lng, lat], zoom: 12, speed: 1.2 });
            marker.togglePopup();
        });
    }

    // 2. Stateless Endpoint Form Post Submission Logic Block
    document.getElementById('register-bridge-form').addEventListener('submit', async function(e) {
        ...
        try {
            ...
            // Add dynamic row elements layout to table body runtime stream
            ...
            newRow.innerHTML = `
                ...
                <td class="py-5 px-6 text-center">
                    <span class="px-2.5 py-1 rounded-md text-[10px] font-black bg-rose-500/10 text-rose-500">0</span>
                </td>
            `;

            tableBody.prepend(newRow);
            
            // Wire the map engine markers bindings to click listeners dynamically
            bindRowTelemetryMarker(newRow);
            
            // Auto fly to target coordinates point immediately upon database registration verification
            const [lat, lng] = coordsString.split(',').map(num => parseFloat(num.trim()));
